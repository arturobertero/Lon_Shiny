...
function(input, output, session) {
  ...
  filtered_data <- reactive({
    ...
    if (!is.null(input$country)) {
      for (val in input$country) { df <- df[grepl(val, df$Country, fixed = TRUE), ] }
    }
    if (!is.null(input$year)) {
      for (val in input$year) { df <- df[df$Year == as.character(val), ] }
    }
    if (!is.null(input$scale)) {
      for (val in input$scale) { df <- df[grepl(val, df$Scale_family, fixed = TRUE), ] }
    }
    if (!is.null(input$topic)) {
      for (val in input$topic) { df <- df[grepl(val, df$Topic, fixed = TRUE), ] }
    }
    df
  })
  
  # 3. Render Table (FIXED FOR EXPORT)
